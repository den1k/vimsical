...
(deftest snapshots-io-test
  (let [expect
    ...
    (testing "ISnapshotStoreChan"
      (is (nil? (<?? (p/insert-snapshots-chan *snapshot-store* (uuid :user) (uuid :vims) expect))))
      (is (= (set expect) (set (<?? (a/into [] (p/select-snapshots-chan *snapshot-store* (uuid :user) (uuid :vims))))))))
    (testing "ISnapshotStoreAync"
      (let [wc     (a/chan 1)
            rc     (a/chan 1)
            _      (p/insert-snapshots-async *snapshot-store* (uuid :user) (uuid :vims) expect (partial a/put! wc) (partial a/put! wc))
            _      (a/<!! wc)
