...
(defn add-edit-event
  ...
  (let [state                   (get state-by-delta-uid delta-uid)
        ...
        vcs'                    (-> vcs
                                    (assoc-in [::state-by-delta-uid (-> deltas' last :uid) ::state.deltas/deltas] all-deltas')
                                    (assoc-in [::state-by-delta-uid (-> deltas' last :uid) ::state.files/state-by-file-uid] files-state-by-file-uid')
                                    (assoc ::timeline timeline'))]
